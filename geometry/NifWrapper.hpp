...
public:
     ...
     */
    template<typename T>
    T* AddBlock(const std::string& name)
    {
        return dynamic_cast<T*>(AddBlock(name));
    }
     ...
     */
    template<typename T>
    nejlika::geometry::Pointer<T*> PointerTo(nejlika::geometry::Construct* block) const
    {
        if (block == nullptr)
        {
     ...
     */
    template<typename T>
    T* FindBlock(const nejlika::geometry::Pointer<T*>& pointer) const
    {
        return pointer.Query(m_Blocks);
    }
