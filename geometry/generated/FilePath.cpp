...
void FilePath::Read(nejlika::Reader& reader, uint32_t version, uint32_t user, uint32_t arg)
{
...
void FilePath::Write(nejlika::Writer& writer, uint32_t version, uint32_t user, uint32_t arg) const
{
    if (version <= VERSION_NUMBER(20, 0, 0, 5))
    {
