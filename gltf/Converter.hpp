...
{
public:
    bool LoadKfm(nejlika::Reader& reader, const std::string& path);

    bool LoadNif(nejlika::Reader& reader, const std::string& path);

    bool LoadKf(nejlika::Reader& reader, const std::string& path);
...
protected:
    ...
    int32_t ExportGLTFHeirarchy(class NiAVObject* node, const std::string& name, tinygltf::Model& model, nejlika::Writer& buffer, int32_t childIndex, int32_t levelOfDetail,
                                bool hasLOD);
    ...
    void ExportAnimation(const std::string& name, const nejlika::geometry::Header& header, const std::vector<nejlika::geometry::Construct*>& blocks, tinygltf::Model& model,
                         nejlika::Writer& buffer);

    void WriteAnimationData(nejlika::Writer& buffer, tinygltf::Model& model, tinygltf::Animation& animation, int32_t nodeIndex, const std::string& targetPath,
